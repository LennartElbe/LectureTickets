<?php
 ...
 $audit_on = 1;
  ...
  function saveDB( $d ) {
     ...
     //syslog(LOG_EMERG, "user 1 is: ".json_encode($d["users"]["1"]));
...
syslog(LOG_EMERG, 'try to remove permission '.$name.' ('.$id.') from '.$role['name']);
  ...
  function check_permission_pair( $permission1, $permission2 ) {
      global $_SESSION;
      ...
      // check if the current user is allowed to upload files
      $roles = list_roles( $user_name );

      $ok = false;
      foreach ( $roles as $role ) {
        $permissions = list_permissions( $role );
        if (in_array($permission1, $permissions) && in_array($permission2, $permissions)) {
	          $ok = true;
        }
      }
      if ($ok) {
        audit( "check_permission pair worked", $permission1." together with ".$permission2." as ".$user_name);
        return true;
      }

      audit( "check_permission pair failed", $permission1." together with ".$permission2." as ".$user_name);
      return false;
  }
